...
// ▸ challenge_started_at / challenge_finished_at выводятся человеку
// ▸ После 14/14 появляется форма «Доказательства шара» (+ feedback API)
...
export default function LK({ user, citizen, progress, notesJSON }) {
  ...
  /* переключение вкладки без полной перегрузки */
  const switchTab = k => {
    setTab(k)
...
<script async src="https://telegram.org/js/telegram-widget.js?15"
 ...
 data-request-access="write" data-auth-url="/api/auth"></script>` }} />
  ...
  /* подгружаем заметки «на лету», чтобы не ломать кэш Next JS ---------- */
  useEffect(()=>{
      ...
      <main style={{ maxWidth:920, margin:'2rem auto', padding:'0 1rem' }}>
        ...
        {tab==='progress' && (
          <>
            ...
            {/* feedback → появляется только после 14/14 */}
            {progress===14 && (
              <form onSubmit={async e=>{
                ...
                <textarea name="fb" rows={4} maxLength={1_000}
