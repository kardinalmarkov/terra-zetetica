      ...
      {!showPreview && !sent && (
        <form onSubmit={(e) => { e.preventDefault(); setShowPreview(true) }} className="bg-white border p-6 rounded-xl shadow-md max-w-3xl">
          <table className="w-full text-sm space-y-2">
            <tbody>
              <tr><td className="pr-4 py-2 font-medium">Ваш Z-ID:</td><td><input name="zid" value={form.zid} onChange={update} required className="input w-full" placeholder="например: ZID-0001" /></td></tr>
              <tr><td className="pr-4 py-2 font-medium">Email:</td><td><input name="email" type="email" value={form.email} onChange={update} required className="input w-full" placeholder="example@site.org" /></td></tr>
              <tr><td className="pr-4 py-2 font-medium">Название анклава:</td><td><input name="name" value={form.name} onChange={update} required className="input w-full" placeholder="например: Дом Единства" /></td></tr>
              <tr>
              ...
              <tr><td className="pr-4 py-2 font-medium">Регион:</td><td><input name="region" value={form.region} onChange={update} required className="input w-full" placeholder="Страна / область / район" /></td></tr>
              <tr><td className="pr-4 py-2 font-medium align-top">Описание:</td><td><textarea name="description" value={form.description} onChange={update} rows={4} required className="textarea w-full" placeholder="Что за анклав, цели, доступ, задачи" /></td></tr>
              <tr>
                ...
                <td>
                  ...
                  <svg ref={svgRef} viewBox="0 0 1000 1000" className="w-full border rounded shadow-md cursor-crosshair"
                    ...
                    {form.coords && (<circle cx={form.coords.x} cy={form.coords.y} r="10" fill="red" />)}
                  </svg>
                </td>
              </tr>
              <tr><td className="pr-4 py-2 font-medium align-top">Правила (если есть):</td><td><textarea name="rules" value={form.rules} onChange={update} rows={2} className="textarea w-full" placeholder="Согласование, доступ, поведение..." /></td></tr>
              <tr><td className="pr-4 py-2 font-medium">Изображение:</td><td><input name="image" value={form.image} onChange={update} placeholder="URL изображения или IPFS CID" className="input w-full" /></td></tr>
            </tbody>
          </table>
