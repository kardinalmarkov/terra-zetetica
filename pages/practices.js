...
const checklistData = {
  ...
  insider: {
    ...
    content: `
      <p><strong>Суть практики:</strong> это путь тишины, наблюдения и принятия. Инсайдер раскрывает, что борьба укрепляет дуальность. Только внутреннее смирение открывает выход из Матрицы.</p>
  ...
  return (
    <>
      ...
      <main className="wrapper" style={{ maxWidth: 900, margin: '0 auto', padding: '2rem 1rem' }}>
        <h1>🌟 Практика внутреннего Пути</h1>
        <p>Отмечай осознанные действия за день. Журнал сохраняется в браузере.</p>
